<template>
  <div class="blog-carousel">
    ...
    <div @mouseover="hovering = true" @mouseleave="hovering = false" ref="blogCarousel" v-swiper:blogSwiper="swiperOption" >
      ...
      >
        <div v-if="item.image" class="swiper-slide" v-for="(item, index) in data" :key="index">
    ...
    <!-- big image slider modal -->
      <!-- <div @keyup.esc="closeCarousel()" class="image-modal" :class="{'is-active': imageModal}">
        <inner-carousel :slide="currentSlide" :images="data"></inner-carousel>
        <span class="cancel-icon" @click="closeCarousel()"></span>

        <div class="counter">
          <p>{{ image + 1 === 0 ? 1 : image + 1 }} / {{ data.length }}</p>
        </div>
...
<script>
  export default {
    ...
    mounted () {
      ...
      // document.querySelector(".journal-carousel").addEventListener("click", event => {
      ...
      document.querySelector(".prev").addEventListener(
        ...
        event => {
          event.preventDefault();
          try {
            this.$refs.blogCarousel.swiper.slidePrev();
          } catch (e) {}
        },
        false
      );
      ...
      document.querySelector(".next").addEventListener(
        ...
        event => {
          event.preventDefault();
          try {
            this.$refs.blogCarousel.swiper.slideNext();
          } catch (e) {}
        },
        false
      );
