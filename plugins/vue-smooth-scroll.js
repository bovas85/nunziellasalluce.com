...
Vue.directive('scroll', {
  // eg. <a href="#header" smooth-scroll>scroll up</a>
  inserted: function (el, binding) {
    ...
    el.addEventListener('click', function (ev) {
      var scrollTo = document.querySelector(binding.value.element)
      if (!scrollTo) return
      ...
      if (window.history.pushState && window.location.hash !== this.hash) {
        window.history.pushState('', '', this.hash)
      }

      // Get duration from element, default to 500ms
      var duration =
        binding.value && binding.value.duration
