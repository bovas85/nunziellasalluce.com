...
const vFormInvalid = computed(() => vNameInvalid.value || vEmailInvalid.value || !form.value.youAgree)
  ...
  <div class="contact-form">
    <form key="notSent" :class="{ 'sending': sent }" class="go-bottom is-flex-column" @submit.prevent>
      <div class="name">
        ...
        <input
id="name" v-model="form.yourName" autocomplete="given-name"
          :class="{ 'is-danger': vNameInvalid && sending || vNameInvalid && nameClicked }" name="name" type="text"
          required @focus="nameFocused = true" @blur="nameClicked = true">
        <i :class="{ 'is-danger': vNameInvalid && sending || vNameInvalid && nameClicked }"><img
src="/images/error.svg"
            alt="error icon"></i>
        <span
:class="{ 'is-visible': vNameInvalid && sending || vNameInvalid && nameClicked }"
          class="has-text-danger">Please type your full name</span>
      </div>

      <div class="email">
        <label :class="{ 'selected': emailFocused || saved }" for="email">Email</label>
        <input
id="email" v-model="form.yourEmail" autocomplete="email"
          :class="{ 'is-danger': vEmailInvalid && sending || vEmailInvalid && emailClicked }" name="email" type="email"
          required @focus="emailFocused = true" @blur="emailClicked = true">
        <i :class="{ 'is-danger': vEmailInvalid && sending || vEmailInvalid && emailClicked }"><img
            src="/images/error.svg" alt="error icon"></i>
        <span
:class="{ 'is-visible': vEmailInvalid && sending || vEmailInvalid && emailClicked }"
          class="has-text-danger">Please type a valid Email</span>
      </div>

      <div class="message">
        <label
:class="{ 'selected': messageFocused || saved }" for="message"
          @focus="messageFocused = true">Message</label>
        <textarea id="message" v-model="form.yourMessage" class="hidden-mobile" rows="5" name="message" />
        <textarea
id="message_mobile" v-model="form.yourMessage" class="is-hidden-mobile-large" rows="1"
          placeholder="Write your message here ..." name="message_mobile" />
      </div>

      <div class="check">
        <input id="check1" v-model="form.youAgree" type="checkbox" required>
        <label
:class="{ 'opacity': form.youAgree, 'is-danger': !form.youAgree && sending }" class="checkbox label"
          for="check1">
          <p>You agree to the <NuxtLink to="/privacy-policy" target="_blank">Privacy Policy</NuxtLink>
          </p>
          <span :class="{ 'is-visible': !form.youAgree && sending }" class="has-text-danger">Please agree with the Privacy
            Policy</span>
        </label>
      </div>
      ...
      <div class="field is-grouped">
        <div class="control">
          ...
          <button
:disabled="disabled || saved" :class="{ 'is-disabled': disabled || saved }"
            class="button button--contact" @click.prevent="sendForm()">{{ isOnline ? 'Submit' : 'Save' }}</button>
        </div>
      </div>

      <p v-show="sendError" class="is-danger col--12 is-left send-error">There was an error sending the form.</p>
      <p v-show="checkValidation" class="is-danger col--12 is-left send-error">There is an error in one of the fields.
        Please review your previous answers</p>
    </form>
