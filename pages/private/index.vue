<template>
  <div class="case-study private" v-if="loadContent && project != null">
    ...
    >
      <div class="container-fluid is-flex">
        <nuxt-link class="previous" :to="`/${previousProject.slug}`">
          <LazyImage
            v-if="
              previousProject.acf.hero != null &&
                previousProject.acf.status === 'true'
            "
            class="image"
            :hover="false"
        ...
        <nuxt-link class="next" :to="`/${nextProject.slug}`">
          <LazyImage
            v-if="
              nextProject.acf.hero != null && nextProject.acf.status === 'true'
            "
            class="image"
            :hover="false"
  ...
  <div style="height: 100vh" v-else-if="project != null">
    <WorkHero :project="project" :animateHeader="animateHeader" />
  </div>
</template>
...
export default {
  ...
  async mounted() {
    ...
    this.loadContent = true; // load after pass check
